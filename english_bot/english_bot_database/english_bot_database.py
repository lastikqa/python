...
class EnglishBotDatabase():
    ...
    def creating_object_user_in_db(self,user_id: int, first_name: str, database_name: str = database_name):
        ...
        connect = sqlite3.connect(database_name)
        cursor = connect.cursor()
        cursor.execute('INSERT INTO Users (user_id, first_name,  translation, game, question,answer, variants, user_score, counting_user_score ) '
                       'VALUES (?, ?, ?, ?, ?, ?,?, ?,?)',
                       (user_id, first_name, "rus", None, None,None, None, 0, 0))
        connect.commit()
        connect.close()
    ...
    def checking_user_translation(self, user_id: int, database_name: str = database_name):
        ...
        try:
            ...
            return None
    def checking_user_variants(self,user_id:int, database_name: str = database_name):
