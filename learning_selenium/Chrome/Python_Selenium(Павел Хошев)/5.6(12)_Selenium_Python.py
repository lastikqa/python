...
with webdriver.Chrome(options=options) as browser:
    ...
    for cookie in cookies:
        ...
        age = browser.find_element(By.CSS_SELECTOR, "div span#age").text.split(":")
        skills = browser.find_elements(By.CSS_SELECTOR, "div li")
        ...
        try:
            new_young_stars[cookie["value"]] = {"age": int(age[-1]), "skills": skill_set}
        except ValueError:
            new_young_stars[cookie["value"]] = {"age": age[-1], "skills": skill_set}
            time.sleep(2)
            browser.delete_all_cookies()

    new_young_stars = sorted(new_young_stars.items(), key=lambda x: ((x[1]["age"]), len(x[1]["skills"])))
    pprint.pprint(new_young_stars[0])
